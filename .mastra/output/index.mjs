...
const parseMetadataBlock = (rawContent) => {
  const metadataMatch = rawContent.match(/<!--([\s\S]*?)-->/);
  if (!metadataMatch) {
    return { metadata: {} };
  }
  const metadataLines = metadataMatch[1].split(/\r?\n/).map((line) => line.trim()).filter((line) => line.length > 0);
  const metadata = {};
...
const readPromptFromFile = async (promptName) => {
  const promptPath = path.join(PROMPTS_DIRECTORY, `${promptName}.md`);
  const rawContent = await promises.readFile(promptPath, "utf8");
  const { metadata, metadataBlock } = parseMetadataBlock(rawContent);
  const contentWithoutMetadata = metadataBlock ? rawContent.replace(metadataBlock, "").trimStart() : rawContent;
...
const loadInstructionsFile = (relativePath) => {
...
const defaultReviewPrompt = loadInstructionsFile(path.join("prompts", "codeReviewDefault.md")) || `\u4F60\u662F\u4E13\u4E1A\u7684\u4EE3\u7801\u8D28\u91CF\u5BA1\u6838\u5458\u3002\u4F18\u5148\u9605\u8BFB\u9879\u76EE\u6307\u4EE4\uFF0C\u5176\u6B21\u53C2\u8003 CODE_STANDARD.md\uFF0C\u518D\u4F7F\u7528\u6B64\u9ED8\u8BA4\u6307\u4EE4\u3002
...
const combinedInstructions = [
  ...
  projectInstructions ? `\u9879\u76EE instructions.md\uFF08\u4F18\u5148\u7EA7\u6700\u9AD8\uFF09\uFF1A
${projectInstructions}` : "\u672A\u627E\u5230\u9879\u76EE instructions.md\uFF0C\u5C06\u76F4\u63A5\u4F9D\u8D56\u9ED8\u8BA4\u4EE3\u7801\u5BA1\u6838\u63D0\u793A\u8BCD\u3002",
  defaultReviewPrompt,
...
`;
const DEFAULT_TECHNOLOGIES = ["nuxt2", "vue2", "midway", "egg"];
const readPackageJson = async () => {
  try {
    const raw = await promises.readFile(PACKAGE_JSON_PATH, "utf8");
...
const getDependencyVersion = (map, name) => map?.[name];
const findDependencyVersion = (pkg, names) => {
  for (const dependencyName of names) {
    const version = getDependencyVersion(pkg.dependencies, dependencyName) ?? getDependencyVersion(pkg.devDependencies, dependencyName);
...
const versionMatchesMajor = (version, major) => {
...
const buildRulesDocument = (technologies) => {
  const techSummary = technologies.map((tech) => tech.inferred ? `${tech.label}\uFF08\u9ED8\u8BA4\u63A8\u8350\uFF09` : tech.label).join("\u3001");
  const intro = technologies.length > 0 ? `> \u6587\u6863\u7531\u4EE3\u7801\u89C4\u8303 MCP \u81EA\u52A8\u751F\u6210\uFF0C\u8BC6\u522B\u5230\u7684\u5173\u952E\u6280\u672F\u6808\uFF1A${techSummary}\u3002` : "> \u6587\u6863\u7531\u4EE3\u7801\u89C4\u8303 MCP \u81EA\u52A8\u751F\u6210\uFF0C\u672A\u80FD\u8BC6\u522B\u5230\u7279\u5B9A\u6280\u672F\u6808\uFF0C\u8BF7\u6309\u5B9E\u9645\u60C5\u51B5\u8865\u5145\u3002";
  const technologySections = technologies.map((tech) => {
  ...
  const technologyBlock = technologySections ? `## \u6280\u672F\u6808\u7279\u5B9A\u89C4\u8303
${technologySections}
` : "";
...
const injectCodeRulesDocumentTool = {
  ...
  execute: async (input = {}) => {
    ...
    const detectedTechnologyDefinitions = useCustomContent ? [] : await detectTechnologies();
    const rulesDocument = useCustomContent ? customContent : buildRulesDocument(detectedTechnologyDefinitions);
    const detectedTechnologies = detectedTechnologyDefinitions.map((tech) => tech.label);
    let fileExists = false;
    try {
      await promises.access(RULES_FILE_PATH);
      fileExists = true;
    } catch (error) {
      if (error.code !== "ENOENT") {
        throw new Error(`\u65E0\u6CD5\u68C0\u67E5 .rules \u6587\u4EF6\u662F\u5426\u5B58\u5728: ${error.message}`);
      }
    }
    ...
    await promises.writeFile(RULES_FILE_PATH, `${rulesDocument.trim()}
`, "utf8");
    return {
...
`,
      message: fileExists ? "\u5DF2\u8986\u76D6\u73B0\u6709\u7684 .rules \u6587\u4EF6\u3002" : "\u5DF2\u751F\u6210\u65B0\u7684 .rules \u4EE3\u7801\u89C4\u8303\u6587\u6863\u3002",
...
const loadKaiInstructions = async () => {
  const kaiPath = path.resolve(process.cwd(), ".kai/instructions.md");
  try {
    return await promises.readFile(kaiPath, "utf8");
  } catch (error) {
    if (error.code === "ENOENT") {
      return void 0;
    }
...
const stageFiles = async (paths) => {
  if (!paths || paths.length === 0) {
    const { stdout: stdout2 } = await runGitCommand(["add", "--all"]);
    return stdout2;
  }
...
const lifecycleGuidance = {
  ...
  testing: {
    key: "testing",
    name: "\u63D0\u6D4B",
    summary: "\u5C06\u5B8C\u6210\u5F00\u53D1\u7684 feature \u5206\u652F\u5408\u5165\u53D1\u5E03\u5206\u652F\uFF0C\u5E76\u6839\u636E\u6D4B\u8BD5\u73AF\u5883\u521B\u5EFA\u6807\u7B7E\u540C\u6B65\u81F3 UAT/Stage\uFF0C\u7B49\u5F85\u6D4B\u8BD5\u786E\u8BA4\u3002",
    ...
    steps: [
      ...
      },
      {
        ...
        notes: "\u5148\u8BBE\u7F6E RELEASE_VERSION/DEPLOY_SEQ/ENVIRONMENT_TAG\uFF0C\u518D\u6267\u884C\u6253\u6807\u3002\u4F8B\u5982 stage-2025.w33.02-01\uFF0C\u53EF\u6309 stage/uat \u533A\u5206\u3002"
      },
      {
  ...
  experience: {
    key: "experience",
    name: "\u4F53\u9A8C",
    summary: "\u4F7F\u7528 release \u5206\u652F\u53CA\u5BF9\u5E94\u6807\u7B7E\u9A8C\u8BC1 stage \u73AF\u5883\uFF0C\u6C89\u6DC0\u4F53\u9A8C\u53CD\u9988\u5E76\u51C6\u5907\u9A8C\u6536\u6750\u6599\u3002",
    ...
    steps: [
      {
        ...
        notes: "\u786E\u4FDD stage \u6807\u7B7E\uFF08\u5982 stage-2025.w33.02-01\uFF09\u5DF2\u90E8\u7F72\uFF0C\u53EF\u901A\u8FC7 STAGE_TAG \u6216\u524D\u8FF0\u73AF\u5883\u53D8\u91CF\u590D\u7528\u547D\u540D\u3002"
      },
      {
        title: "\u8DDF\u8FDB\u4F53\u9A8C\u53CD\u9988",
        commands: [
  ...
  acceptance: {
    key: "acceptance",
    name: "\u9A8C\u6536",
    summary: "\u901A\u8FC7 MR \u5C06 release \u5206\u652F\u5408\u5165 master\uFF0C\u5E76\u5728 pre-production \u73AF\u5883\u5B8C\u6210\u56DE\u5F52\u9A8C\u8BC1\u3002",
  ...
  release: {
    ...
    steps: [
      ...
      },
      {
        ...
        notes: "\u901A\u8FC7 export \u6307\u5B9A\u7248\u672C\u4E0E\u53D1\u5E03\u53F7\u540E\u518D\u6267\u884C\u6253\u6807\uFF0C\u4F8B\u5982 production-2025.w33.02-01\uFF0C\u5E76\u5728\u4E0A\u7EBF\u64CD\u4F5C\u540E\u8BB0\u5F55\u4EA7\u7EBF\u65F6\u95F4\u70B9\u3002"
      }
    ],
    reminders: [
  ...
  operation: {
    key: "operation",
    name: "\u8FD0\u8425",
    summary: "\u9762\u5BF9\u4EA7\u7EBF\u95EE\u9898\uFF0C\u4F18\u5148\u8BC4\u4F30\u662F\u5426\u56DE\u6EDA\u6807\u7B7E\uFF0C\u518D\u9009\u62E9 bugfix \u6216 hotfix \u5206\u652F\u4FEE\u590D\u5E76\u56DE\u704C\u4E3B\u5E72\u3002",
...
const getActionGuidance = (action, stageDefinition) => {
  ...
  return {
    reminders: Array.from(/* @__PURE__ */ new Set([...stageReminders, ...base.reminders])),
    recommendedCommands: Array.from(/* @__PURE__ */ new Set([...stageRecommended, ...base.recommendedCommands]))
  };
};
const branchComplianceRules = [
...
const formatLifecycleOutput = (definition) => {
...
const evaluateBranchCompliance = (branch, stageDefinition) => {
  if (!branch) {
    return void 0;
  }
...
const gitWorkflowTool = {
  ...
  execute: async ({ action, options, preCommitReview }) => {
    ...
    const stageDefinition = stageKey ? lifecycleGuidance[stageKey] : void 0;
    ...
    switch (action) {
      ...
      case "pull": {
        const remote = options?.remote ? String(options.remote) : void 0;
        const branch = options?.branch ? String(options.branch) : void 0;
        const args = ["pull"];
        if (remote) {
          args.push(remote);
        }
      ...
      case "createBranch": {
        ...
        const fromBranch = options?.from ? String(options.from) : void 0;
      ...
      case "merge": {
        const sourceBranch = options?.source ? String(options.source) : void 0;
        if (!sourceBranch) {
      ...
      case "commit": {
        const message = options?.message ? String(options.message) : void 0;
        if (!message) {
        ...
        if (preCommitReview?.enabled && !preCommitReview.approval) {
          ...
          review = {
            ...
            diff,
            notes: "\u5DF2\u5F00\u542F\u63D0\u4EA4\u524D\u5BA1\u67E5\u3002\u8BF7\u6839\u636E combinedPrompt \u7684\u89C4\u8303\u68C0\u67E5 diff \u5185\u5BB9\uFF0C\u901A\u8FC7\u540E\u91CD\u65B0\u6267\u884C\u5E76\u8BBE\u7F6E preCommitReview.approval=true\u3002"
          };
    ...
    return {
      ...
      review,
      branchCompliance,
      lifecycle: action === "lifecycleGuide" ? lifecycleInfo : void 0
    };
  }
};
...
const DEFAULT_REFRESH_MARGIN = 60 * 1e3;
...
const getSetCookieHeaders = (headers) => {
  const rawHeaders = headers.getSetCookie?.();
  ...
  for (const attribute of attributeSegments) {
    ...
    if (lowerName === "max-age" && attributeValue) {
      ...
      if (!Number.isNaN(seconds)) {
        stored.expiresAt = Date.now() + seconds * 1e3;
      }
    }
    if (lowerName === "expires" && attributeValue && stored.expiresAt === void 0) {
...
const decodeJwtExpiration = (token) => {
  ...
  try {
    const decoded = Buffer$1.from(padded, "base64").toString("utf-8");
...
class GrafanaMcpClient {
  ...
  async requestWithRedirect(path, init = {}, redirectCount = 0) {
    ...
    if (this.isRedirect(response.status)) {
      ...
      let nextBody = init.body ?? void 0;
      if (response.status === 303 || (response.status === 301 || response.status === 302) && nextMethod !== "GET" && nextMethod !== "HEAD") {
        nextMethod = "GET";
        nextBody = void 0;
      }
      const nextInit = {
  ...
  async authorizedFetch(path, init = {}, attempt = 0) {
  ...
  async getPanelDefinition(uid, panelId) {
  ...
  async safeReadError(response) {
    try {
      ...
      try {
        ...
        return `Unable to read error response: ${innerError.message}`;
      }
    }
  }
  findPanel(obj, panelId) {
    if (!obj) {
      return void 0;
    }
...
const grafanaClientCache = /* @__PURE__ */ new Map();
const decodeServiceAccountJson = (input) => {
  try {
    return JSON.parse(input);
  } catch (error) {
    try {
      const decoded = Buffer$1.from(input, "base64").toString("utf-8");
...
const resolveGrafanaConfig = (credentials) => {
  ...
  const rawServiceAccountJson = credentials?.serviceAccountJson ?? process.env.GRAFANA_SERVICE_ACCOUNT_JSON ?? process.env.GRAFANA_GOOGLE_CREDENTIALS;
  let clientEmail = credentials?.googleClientEmail ?? process.env.GRAFANA_GOOGLE_CLIENT_EMAIL ?? process.env.GRAFANA_CLIENT_EMAIL;
  let privateKey = credentials?.googlePrivateKey ?? process.env.GRAFANA_GOOGLE_PRIVATE_KEY ?? process.env.GRAFANA_PRIVATE_KEY;
  ...
  const targetAudience = credentials?.googleTargetAudience ?? process.env.GRAFANA_GOOGLE_TARGET_AUDIENCE ?? process.env.GRAFANA_IAP_TARGET_AUDIENCE ?? void 0;
  return {
    baseUrl,
    googleAuth: {
      clientEmail,
...
const getGrafanaClient = (credentials) => {
...
const resolveTaxonomy = (overrides) => ({
...
class SentrySessionManager {
  ...
  async saveSessionCookie(cookie) {
...
class SentryMcpClient {
  ...
  async fetchIssues(limit = this.defaultLimit) {
  ...
  buildTaxonomy(overrides) {
    return resolveTaxonomy(overrides);
  }
  annotateIssues(issues, annotations, overrides) {
    ...
    return issues.map((issue) => {
      ...
      const risk = annotation.riskId ? riskMap.get(annotation.riskId) : void 0;
      const type = annotation.issueTypeId ? typeMap.get(annotation.issueTypeId) : void 0;
      const freq = annotation.frequencyBandId ? freqMap.get(annotation.frequencyBandId) : void 0;
      return {
        ...issue,
        ...annotation,
    ...
    switch (action) {
      ...
      case "notifyHighRisk": {
        ...
        const highRiskIds = new Set(
          taxonomy.riskLevels.filter((entry) => entry.severity === "high" || entry.severity === "critical").map((entry) => entry.id)
        );
        ...
        if (notificationConfig?.larkWebhook) {
          const payload = {
            msg_type: "text",
            content: {
              text: notificationConfig.larkTemplate ?? `Sentry \u9AD8\u98CE\u9669\u544A\u8B66\uFF08${highRiskIssues.length} \u6761\uFF09\uFF1A
` + highRiskIssues.map((item) => `${item.title} [${item.riskLabel ?? item.riskId}] -> ${item.permalink ?? ""}`).join("\n")
            }
          };
          try {
        ...
        if (notificationConfig?.emailRecipients?.length) {
          ...
          const grouped = highRiskIssues.reduce((acc, issue) => {
            const key = notificationConfig.groupBy === "issueType" ? issue.issueTypeLabel ?? issue.issueTypeId ?? "unknown" : issue.riskLabel ?? issue.riskId ?? "unknown";
            acc[key] = acc[key] ?? [];
            acc[key].push(issue);
            return acc;
          }, {});
          const emailBody = Object.entries(grouped).map(([group, items]) => {
            const lines = items.map((item) => `- ${item.title} (${item.frequency ?? "?"} \u6B21) ${item.permalink ?? ""}`).join("\n");
            return `\u3010${group}\u3011
${lines}`;
          }).join("\n\n");
          notifications.push({
