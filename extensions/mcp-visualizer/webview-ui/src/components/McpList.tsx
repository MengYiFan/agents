        ...
        <div className="p-4 space-y-8">
            <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {docs.map(doc => (
                        <div key={doc.id} className="border rounded-lg p-4 bg-card hover:bg-accent/50 transition-colors cursor-pointer"
                             onClick={() => vscode.postMessage({ type: 'openDoc', docId: doc.id, language: doc.defaultLanguage })}>
                ...
                <div className="space-y-2">
                    {instructions.map(item => (
                        <div key={item.id} className="flex items-center justify-between p-3 border rounded-md bg-card">
                            ...
                            <Button size="sm" variant="secondary" onClick={() => vscode.postMessage({ type: 'executeInstruction', instructionId: item.id })}>
                                {item.actionLabel}
                            </Button>
                        </div>
                    ))}
